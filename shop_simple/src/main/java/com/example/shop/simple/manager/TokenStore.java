...
import com.example.shop.simple.constants.OauthCacheNames;
import com.example.shop.simple.exception.ShopBindException;
import com.example.shop.simple.pojo.bo.TokenInfoBO;
...
import com.example.shop.simple.serializer.KryoRedisSerializer;
import com.example.shop.simple.utils.PrincipalUtil;
...
public class TokenStore {
    ...
    public UserInfoInTokenBO getUserInfoByAccessToken(String accessToken, boolean needDecrypt) {
        if (StrUtil.isBlank(accessToken)) {
            throw new ShopBindException(HttpStatus.UNAUTHORIZED, "accessToken is blank");
        }
        ...
        if (userInfoInTokenBO == null) {
            throw new ShopBindException(HttpStatus.UNAUTHORIZED, "accessToken 已过期");
        }
    ...
    public TokenInfoBO refreshToken(String refreshToken) {
        if (StrUtil.isBlank(refreshToken)) {
            throw new ShopBindException(HttpStatus.UNAUTHORIZED, "refreshToken is blank");
        }
        ...
        if (StrUtil.isBlank(accessToken)) {
            throw new ShopBindException(HttpStatus.UNAUTHORIZED, "refreshToken 已过期");
        }
    ...
    private String decryptToken(String data) {
        ...
        try {
            ...
            if (System.currentTimeMillis() - createTokenTime > expiresIn * second) {
                throw new ShopBindException(HttpStatus.UNAUTHORIZED, "token error");
            }
        } catch (Exception e) {
            throw new ShopBindException(HttpStatus.UNAUTHORIZED, "token error");
        }
